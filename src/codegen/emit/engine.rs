...
impl<'a> Emitter<'a> {
    ...
    fn flush_comments(&mut self, until_line: usize, prefix: &str) {
        while self.comment_idx < self.pending_comments.len() {
            let comment = &self.pending_comments[self.comment_idx];
    ...
    fn source_mark(&self, line: usize) -> String {
        let mapped = if line == 0 {
            1
        } else {
            crate::preprocess::remap_line(&self.ctx.line_map, line)
        };
        let name = Path::new(&self.program.file_name)
            .file_name()
            .map(|n| n.to_string_lossy().into_owned())
            .unwrap_or_else(|| self.program.file_name.clone());
        format!("-- {name}:{mapped}")
    }
    ...
    fn emit_function(&mut self, func: &Function) {
        ...
        let params = self.param_list(func);
        let gens = if func.type_params.is_empty() {
            String::new()
        } else {
    ...
    fn emit_stmt(&mut self, stmt: &Stmt, indent: usize) -> Vec<String> {
        ...
        match stmt {
            ...
            // Intent comptime: erased from Luau (checked at compile time only).
            Stmt::Comptime { .. } => Vec::new(),
    ...
    fn emit_expr(&self, expr: &Expr) -> String {
        match expr {
            ...
            Expr::Call {
                ...
                if object.is_none() && name == "GetService" {
                    ...
                    return format!("game:GetService(\"{service}\")");
                }
            ...
            Expr::Binary { op, left, right } => {
                ...
                if op == ".:" {
                    return format!(
                        "{} .. {}",
                        self.emit_concat_part(left),
                        self.emit_concat_part(right)
                    );
                }
                if op == "**" {
    ...
    fn emit_concat_part(&self, expr: &Expr) -> String {
        match expr {
            Expr::String(_) | Expr::Interp { .. } => self.emit_expr(expr),
            Expr::Binary { op, .. } if op == ".:" => self.emit_expr(expr),
            Expr::Member { .. }
            | Expr::Call { .. }
            | Expr::AtField { .. }
            | Expr::This { .. }
            | Expr::Cast { .. }
            | Expr::OptionalChain { .. } => self.emit_expr(expr),
            _ => format!("tostring({})", self.emit_expr(expr)),
        }
    }
    ...
    fn emit_call(
        ...
        if object.is_none() && name == "Some" {
            return args
                .first()
                .map(|a| self.emit_expr(a))
                .unwrap_or_else(|| "nil".into());
        }
        if object.is_none() && name == "Ok" {
            let inner = args
                .first()
                .map(|a| self.emit_expr(a))
                .unwrap_or_else(|| "nil".into());
